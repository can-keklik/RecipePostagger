...
for pair in brown.tagged_words():
    word_lowered = pair[0].lower()
    if word_lowered not in brown_lookup:
        brown_lookup[word_lowered] = []
    brown_lookup[word_lowered].append(pair[1])
...
def giveTheMostCommonTag(tokenizedWords):
    ...
    for i in xrange(len(tokenizedWords)):
        ...
        if lem in brown_lookup:
            candidates = brown_lookup[lem]
        else:
            candidates = []
        table = nltk.FreqDist(candidates)
